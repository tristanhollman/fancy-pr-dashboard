...
const concepts = {
  workbench: {
    label: '01 - Workbench', title: 'A home base for daily review.',
  ...
  radar: {
    label: '02 - Repo Radar', title: 'The team, at a glance.',
  ...
  focus: {
    label: '03 - Focus Inbox', title: 'Read before you context-switch.',
...
function initialState() {
  return {
    concept: 'workbench', width: 'wide', scenario: 'populated', scope: 'team', repo: 'all',
...
function filteredData() {
  ...
  const visible = source.filter(pr => {
    ...
    if (state.hideReviewed && pr.personal === 'approved') { hidden.reviewed++; return false; }
    return true;
  });
  const scoped = visible.filter(pr => scopeMatches(pr, state.scope));
  ...
  return {visible, scoped, rows, hidden};
}
...
function navigationRows() {
  return groupRows(filteredData().rows).flatMap(group => group.rows);
}
...
function inspectorHtml(pr) {
  ...
  return `<aside class="panel inspector" aria-label="Selected PR details">
    <h2 class="panel-heading"><span><span class="panel-index">03</span>PR inspector</span><button class="back-button" data-action="back" data-focus="back">[p] Back to queue</button><small>!${pr.id}</small></h2>
...
function railHtml(scoped) {
  ...
  return `<aside class="panel repo-rail" aria-label="Repository filter">
    <h2 class="panel-heading"><span><span class="panel-index">01</span>Repositories</span></h2>
...
function queueHtml(rows) {
  ...
  return `<section class="panel queue-panel" aria-label="Pull request queue">
    <h2 class="panel-heading"><span class="queue-heading-label"><span><span class="panel-index">02</span>${card ? 'Review inbox' : scopes[state.scope]}</span>${card ? '<small>Pick a change. Get the context.</small>' : ''}</span><small>${rows.length} open</small></h2>
    ${card ? '' : '<div class="queue-columns"><span>PULL REQUEST / YOUR ACTION</span><span>REQUIRED</span><span>AGE</span></div>'}
    <div class="scroll" data-scroll="queue">${groupRows(rows).map(group => `<section aria-label="${escapeHtml(group.key)}"><h3 class="repo-group-title"><span>${escapeHtml(group.repo)} <small>${group.rows.length}</small></span><small>${escapeHtml(group.project)}</small></h3>${group.rows.map(pr => rowHtml(pr, card)).join('')}</section>`).join('')}</div>
    <div class="queue-bottom"><span><strong>${ordered.findIndex(pr => pr.id === state.selected) + 1}</strong> / ${rows.length} selected</span><span>${card ? 'required = approved entries' : 'grouped by repo · scroll for more'}</span></div>
  </section>`;
}
...
function radarHtml(rows, selected) {
  ...
  return `<div class="radar-grid ${groups.length === 1 ? 'single-repo' : ''}" data-scroll="radar-grid">
    ${groups.map((group, index) => `<section class="panel radar-panel" aria-label="${escapeHtml(group.key)}">
      <h2 class="panel-heading"><span><span class="panel-index">0${index + 1}</span>${escapeHtml(group.repo)}</span><small>${escapeHtml(group.project)}</small></h2>
...
function render() {
  ...
  const {visible, scoped, rows, hidden} = filteredData();
  if (!rows.some(pr => pr.id === state.selected)) state.selected = groupRows(rows)[0]?.rows[0]?.id ?? null;
  ...
  app.innerHTML = `
    ...
    <nav class="scope-bar" aria-label="Queue scope">${scopeNames.map(([key, label], index) => `<button class="scope ${state.scope === key ? 'active' : ''}" data-action="scope" data-scope="${key}" data-focus="scope-${key}" aria-pressed="${state.scope === key}" title="${index + 1}: ${label}">${label}<span class="count">${loading ? '--' : visible.filter(pr => scopeMatches(pr, key)).length}</span></button>`).join('')}</nav>
    <div class="queue-tools">
      ...
      <select class="repo-select" data-focus="repo-select" id="repo-select" aria-label="Repository filter" ${state.concept !== 'workbench' ? 'style="display:block"' : ''}><option value="all">All repositories</option>${groups.map(group => `<option value="${escapeHtml(group.key)}" ${state.repo === group.key ? 'selected' : ''}>${escapeHtml(group.project)} / ${escapeHtml(group.repo)}</option>`).join('')}</select>
      <select class="sort-select" data-focus="sort" id="sort" aria-label="Sort pull requests within each repository"><option value="newest" ${state.sort === 'newest' ? 'selected' : ''}>Newest first</option><option value="oldest" ${state.sort === 'oldest' ? 'selected' : ''}>Oldest first</option></select>
      <div class="filter-buttons">${filterButton('d', 'hideDrafts', 'Drafts')}${filterButton('b', 'hideBots', 'Bots')}${filterButton('h', 'hideReviewed', 'Reviewed')}</div>
      <button class="detail-toggle" data-action="details" data-focus="details" aria-pressed="${isCompact() ? state.compactDetails : state.details}">[p] Details</button>
    </div>
    <div class="filter-status"><span>${loading ? 'Waiting for data' : `<strong>${rows.length} visible</strong> · ${hidden.drafts} draft / ${hidden.bots} bot / ${hidden.reviewed} reviewed hidden across scopes`}</span><span class="selection-count">${selected ? `${ordered.findIndex(pr => pr.id === selected.id) + 1} / ${rows.length} selected` : ''}</span></div>
    ...
    <div class="board ${state.concept} ${state.details ? '' : 'no-inspector'} ${state.compactDetails && selected ? 'inspector-open' : ''} ${state.expanded ? 'expanded' : ''}">
      ${loading || !selected ? emptyHtml(loading) : state.concept === 'radar' ? radarHtml(rows, selected) : `${state.concept === 'workbench' ? railHtml(scoped) : ''}${queueHtml(rows)}${state.details ? inspectorHtml(selected) : ''}`}
    </div>
    <footer class="terminal-footer"><span><kbd>j k</kbd> move</span><span><kbd>1-4</kbd> scope</span><span><kbd>/</kbd> find</span><span><kbd>p</kbd> details</span><span><kbd>enter</kbd> browser</span><span><kbd>r</kbd> refresh</span><span class="footer-right">required = approved entries / total required</span></footer>`;
...
document.addEventListener('click', event => {
  ...
  if (button.id === 'reset') {
    ...
    terminal.dataset.width = state.width;
    render();
    notify('Study reset to Workbench, with default filters and fictional data.');
    return;
  }
  switch (button.dataset.action) {
    ...
    case 'clear': state.query = ''; state.repo = 'all'; state.hideDrafts = false; state.hideBots = false; state.hideReviewed = false; render(); break;
  }
});
